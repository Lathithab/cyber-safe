    ...
    <main className="feed-dashboard">
      <aside className="sidebar">
        ...
        <nav className="side-nav" aria-label="Dashboard navigation">
          ...
          {secondaryLinks.map(([label, route, icon]) => <button key={label} className="side-link" type="button" onClick={() => router.push(route)}><DashboardNavIcon name={icon} size={24} /><span>{label}</span></button>)}
        </nav>
        ...
        <button className="user-chip" type="button" onClick={() => router.push("/login")}><span className="chip-avatar">SN</span><span><strong>{CURRENT_USER.name}</strong><small>{CURRENT_USER.role}</small></span></button>
      </aside>
