    ...
    <main className="settings-dashboard">
      <aside className="sidebar">
        ...
        <nav className="side-nav" aria-label="Dashboard navigation">
          ...
          {secondaryLinks.map(([label, route, icon]) => <button key={label} className={`side-link ${route === "/settings" ? "active" : ""}`} type="button" onClick={() => router.push(route)}><DashboardNavIcon name={icon} size={24} /><span>{label}</span></button>)}
        </nav>
